...
const CreatePost = () => {
    ...
    const handlePostCreation = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const response = await fetch('/apicrud/create-post', {
            method: 'POST',
            headers: {
    ...
    return (
        <div className='w-full max-w-6xl mx-auto text-center h-screen'>
            <h1 className='font-bold text-2xl mt-20 mb-10'>Review for our page</h1>
            ...
            <div>
                <button className='text-white-900 leading-7 mt-4 underline hover:text-gray-900/70' onClick={() => router.push('/comment')}>Go to Books</button>
            </div>
        </div>
    )
}
